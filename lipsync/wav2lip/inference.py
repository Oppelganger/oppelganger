...
def load_blazeface_model(path: str) -> mpv.FaceDetector:
...
wav2lip_model: Wav2Lip = load_wav2lip_model('models/wav2lip.pth')
print('Model Wav2Lip loaded')

print('Loading BlazeFace model')
blazeface_model: mpv.FaceDetector = load_blazeface_model('models/blazeface.tflite')
print('Model BlazeFace loaded')

print('Loading GFPGAN model')
gfpgan_model: gfpgan.GFPGANer = gfpgan.GFPGANer(
	model_path='models/gfpgan.pth',
	device=device
)
print('Model GFPGAN loaded')
...
def face_detect(
	...
	for image in images:
		...
		detections = blazeface_model.detect(image).detections
...
def wav2lip(
	...
	with torch.no_grad():
		for i, (img_batch, mel_batch, frames, coords) in enumerate(gen):
			...
			preds = wav2lip_model(mel_batch, img_batch)
